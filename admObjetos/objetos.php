...
<head>
    ...
    <title>Objetos | E.E.T.P 462</title>
</head>
<body>
    ...
    <div class="container">
        <div class="row mt-4">
            <div class="col-12 col-sm-4">
                ...
                <div class="mb-4 mt-5">
                    <a type="button" class="btn btn-primary addBtn col-12" href="php/regObjeto.php">Agregar Objeto</a>
                </div>
                
                <div class="mb-4">
                    <a type="button" id="editBtn" class="btn btn-secondary col-12" href="#">Editar Objeto</a>
                </div>
                ...
                <div class="mb-4">
                    <a type="button" class="btn btn-warning col-12" href="../admMoves/Objetos/tabladeobjetos.php">Movimientos</a>
                </div>
                
                <div class="mb-4">
                    <a type="button" id="delButton" class="btn btn-danger col-12" href="php/delObjeto.php">Eliminar Objeto</a>
                </div>
            </div>
            ...
            <div class="col-12 col-sm-8 table-responsive table-wrapper">
                <table class="table text-center" id="table" style="border: 1px solid #303436;">
                    ...
                    <tbody>
                        <?php
                        $consulta = "SELECT * FROM tabladeobjetos ORDER BY objeto ASC";
                        $resultado = mysqli_query($connection, $consulta);

                        if(mysqli_num_rows($resultado) > 0) {
                            // Si hay datos, mostrarlos en una tabla HTML
                            while($fila = mysqli_fetch_assoc($resultado)) {
                                echo "<tr class='fila-tabla' data-objeto='" . $fila['objeto'] . "' id='" . $fila['objeto'] . "'>";
                                echo "<td>" . $fila['objeto'] . " (" . $fila['id'] . ")</td>";
    ...
    <script>
        // Manejador de eventos para el botón "Editar Objeto"
        $("#editBtn").click(function() {
            ...
            var objeto = selectedRow.find("td:eq(0)").text();
            var existencias = selectedRow.find("td:eq(1)").text();
            var observaciones = selectedRow.find("td:eq(2)").text();
            // Crea el formulario de edición y lo inserta después de la fila seleccionada
            var editForm = $("<tr class='edit-form'><td><input type='text' id='inputobjeto' name='objeto' value='" + objeto + "' class='edit-input'></td><td><input type='text' id='inputexistencias' name='existencias' value='" + existencias + "' class='edit-input text-center'><br><button class='save-btn mt-3 mb-3'>Guardar</button></td><td><input type='text' id='inputobservaciones' name='observaciones' value='" + observaciones + "' class='edit-input'></td></tr>");
        ...
        $(document).on("click", ".save-btn", function() {
            ...
            var objeto = $(".edit-form input[name='objeto']").val();
            var existencias = $(".edit-form input[name='existencias']").val();
            var observaciones = $(".edit-form input[name='observaciones']").val();
            // Actualiza la fila de la tabla con los nuevos valores editados
            selectedRow.find("td:eq(0)").text(objeto);
            selectedRow.find("td:eq(1)").text(existencias);
            selectedRow.find("td:eq(2)").text(observaciones);
            ...
            $.ajax({
                type: "POST",
                url: "php/updateObjeto.php",
                data: {id: id, objeto: objeto, existencias: existencias, observaciones: observaciones},
